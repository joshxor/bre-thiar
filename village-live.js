...
async function textImage(path){const r=await fetch(path,{cache:'force-cache'});if(!r.ok)throw new Error('Asset '+r.status+' '+path);const s=(await r.text()).trim();return image('data:image/webp;base64,'+s);}
async function chunkImage(parts){let s='';for(const f of parts){const r=await fetch(f,{cache:'force-cache'});if(!r.ok)throw new Error('Asset '+r.status+' '+f);s+=(await r.text()).trim();}return image('data:image/webp;base64,'+s);}
async function load(){try{$('status').textContent='Loading Bré Thiar production art…';bg=await chunkImage(BG_PARTS);let i=0;for(const n of NPCS){npcImgs[n.id]=await image(n.file);$('status').textContent='Loading village entities… '+(++i)+'/4';}try{playerImg=await textImage('mobile/player_q85.b64');}catch(e){console.warn('Player cache fallback',e);try{playerImg=await image('assets/mobile/player.webp');}catch(_){playerImg=npcImgs.brannoc;}}$('status').textContent='Ready — tap Enter Bré Thiar';$('enter').disabled=false;ready=true;}catch(e){console.error(e);$('status').textContent='Load failed — '+e.message;}}
...
function step(dt){const d=activeDirection();if(!d){frame=0;return;}p.f=d;const v=86,dx=d==='left'?-v*dt:d==='right'?v*dt:0,dy=d==='up'?-v*dt:d==='down'?v*dt:0,nx=p.x+dx,ny=p.y+dy;if(valid(nx,ny)){p.x=nx;p.y=ny;walk+=Math.hypot(dx,dy);frame=(walk/8|0)%4;localStorage.setItem('breThiarVillageQA',JSON.stringify(p));}}
...
$('atk').onclick=()=>toast('Combat unlocks with the Rowanwood mobile slice.');$('menu').onclick=()=>$('sheet').classList.remove('hide');$('close').onclick=()=>$('sheet').classList.add('hide');$('dc').onclick=()=>$('dialog').classList.add('hide');$('reset').onclick=()=>{p={...SPAWN};walk=0;frame=0;localStorage.removeItem('breThiarVillageQA');$('sheet').classList.add('hide');toast('Position reset.');};
