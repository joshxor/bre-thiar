...
SKIP_DIR_NAMES = {".git", ".godot", "node_modules", "__pycache__"}
KNOWN_REJECT_BASENAMES = {"village.webp", "village_front.webp", "player.webp"}
...
def match_target(fmt: str, width: int, height: int) -> Target | None:
    return next((target for target in TARGETS if target.fmt == fmt and target.width == width and target.height == height), None)
...
def candidate_from_bytes(*, data: bytes, source: str, container: str, entry: str | None) -> Candidate | None:
    try:
...
def scan_regular_image(path: Path) -> Candidate | None:
    ...
    try:
        ...
        return None
    return candidate_from_bytes(data=data, source=str(path), container="file", entry=None)
...
def main() -> int:
    ...
    parser.add_argument("roots", nargs="+", type=Path, help="Directories, image files, or ZIP archives to scan")
    ...
    if args.json:
        ...
        for candidate in matches:
            print(
                f"MATCH {candidate.target}: {candidate.source}\n"
