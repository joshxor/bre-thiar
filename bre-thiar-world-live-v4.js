'use strict';
const BUILD='hypnobius-world-v4.0';
...
function renderProfile(){
  const zone=zoneInfo()||{};
  const profile=manifest.renderProfiles?.[zone.renderProfile]||{};
  return{
    terrain:profile.terrain||manifest.terrain,
    tileLayers:profile.tileLayers||['Ground','Swamp','Roads'],
    worldTiles:profile.worldTiles||manifest.worldTiles,
    worldTilesetMatch:profile.worldTilesetMatch||'world_objects',
    worldFirstGid:profile.worldFirstGid||6
  }
}
...
function parseMap(){
  const rp=renderProfile();
  const wf=(map.tilesets.find(t=>String(t.source||'').includes(rp.worldTilesetMatch))||{firstgid:rp.worldFirstGid}).firstgid;
...
function drawTerrain(c){
  const rp=renderProfile(),terrain=rp.terrain;
  ...
  const im=worldAtlases[terrain.atlas],[ax,ay]=terrain.rect,sw=terrain.sourceTileWidth,sh=terrain.sourceTileHeight;
  for(const lname of rp.tileLayers){
    ...
    for(let ty=fr;ty<=lr;ty++)for(let tx=fc;tx<=lc;tx++){
      ...
      const id=gid-1,sx=ax+(id%terrain.columns)*sw,sy=ay+Math.floor(id/terrain.columns)*sh,[dx,dy]=screen(tx*ts,ty*ts,c);
      ctx.drawImage(im,sx,sy,sw,sh,Math.floor(dx),Math.floor(dy),Math.ceil(ts*c.z)+1,Math.ceil(ts*c.z)+1)
    }
  }
}
function drawWorld(o,c){
  const tiles=renderProfile().worldTiles;
  const m=tiles[String(o.localId)];if(!m)return;
